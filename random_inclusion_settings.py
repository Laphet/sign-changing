...
if __name__ == "__main__":
    ...
    print_log("=" * 80)
    print_log("Start")
    print_log("In the medium, sigma-inc={0:.4e}, sigma-mat={1:.4e}".format(*sigma_im))
    ...
    np.save(
        "{0:s}/{1:s}".format(DAT_ROOT_PATH, "random-inclusion{:s}.npy".format(token)),
        u_fem,
    )
...
if __name__ == "__main__plot_fig__":
    ...
    ax = axs2[0]
    ax.plot(xaxis_ticks, error_sigma0[0, :], "D-", label="$H=1/10$")
    ax.plot(xaxis_ticks, error_sigma0[1, :], "d-", label="$H=1/20$")
    ax.plot(xaxis_ticks, error_sigma0[2, :], "H-", label="$H=1/40$")
    ax.plot(xaxis_ticks, error_sigma0[3, :], "h-", label="$H=1/80$")
    ...
    ax = axs2[1]
    ax.plot(xaxis_ticks, error_sigma0[4, :], "D-", label="$H=1/10$")
    ax.plot(xaxis_ticks, error_sigma0[5, :], "d-", label="$H=1/20$")
    ax.plot(xaxis_ticks, error_sigma0[6, :], "H-", label="$H=1/40$")
    ax.plot(xaxis_ticks, error_sigma0[7, :], "h-", label="$H=1/80$")
    ...
    ax = axs2[2]
    ax.plot(xaxis_ticks, error_sigma1[0, :], "D-", label="$H=1/10$")
    ax.plot(xaxis_ticks, error_sigma1[1, :], "d-", label="$H=1/20$")
    ax.plot(xaxis_ticks, error_sigma1[2, :], "H-", label="$H=1/40$")
    ax.plot(xaxis_ticks, error_sigma1[3, :], "h-", label="$H=1/80$")
    ax.set_ylabel("Relative errors in energy norm")
    ax.set_yscale("log")
    ax.set_xlabel("$l^*$")
    ax.set_xticks(xaxis_ticks, xaxis_labels)
    ...
    ax = axs2[3]
    ax.plot(xaxis_ticks, error_sigma1[4, :], "D-", label="$H=1/10$")
    ax.plot(xaxis_ticks, error_sigma1[5, :], "d-", label="$H=1/20$")
    ax.plot(xaxis_ticks, error_sigma1[6, :], "H-", label="$H=1/40$")
    ax.plot(xaxis_ticks, error_sigma1[7, :], "h-", label="$H=1/80$")
    ax.set_ylabel("Relative errors in $L^2$ norm")
    ax.set_yscale("log")
    ax.set_xlabel("$l^*$")
    ax.set_xticks(xaxis_ticks, xaxis_labels)

    axs2[0].set_title("$(\sigma_*^+, \sigma_*^-)=(1.0, 1.0e-3)$")
    axs2[2].set_title("$(\sigma_*^+, \sigma_*^-)=(1.0, 1.0e+3)$")
    ...
    for i in range(4):
        print(
            ...
            *[
                r"\num{"
                + "{:.3e}".format(eigen_sigma0[i, j, 0])
                + "}"
                + r" & \num{"
                + "{:.3e}".format(eigen_sigma0[i, j, 1])
                + "}"
                for j in range(1, 4)
            ],
            sep=" & ",
    ...
    for i in range(4):
        print(
            ...
            *[
                r"\num{"
                + "{:.3e}".format(eigen_sigma1[i, j, 0])
                + "}"
                + r" & \num{"
                + "{:.3e}".format(eigen_sigma1[i, j, 1])
                + "}"
                for j in range(1, 4)
            ],
            sep=" & ",
